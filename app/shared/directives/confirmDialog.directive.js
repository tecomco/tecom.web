...
app.directive('confirmDialog', function() {
  ...
  function getTemplate(element, attrs) {
    var templateString = '<button class="{{class}}" ' +
      'mwl-confirm message="{{message}}" confirm-text="{{ok}}" animation="true" cancel-text="{{cancel}}" placement="bottom" on-confirm="callUpdate()" confirm-button-type="{{confirmButton}}" cancel-button-type="default">';
    if (attrs.icon)
